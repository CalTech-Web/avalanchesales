...
- `avalanchesales` — project folder name, lowercase, no spaces (e.g. `davidsplumbing`)
- `DOMAIN` — client's existing website URL (e.g. `davidsplumbing.com`), or `none` if they don't have one
- `PROJECT_PATH` — `/Users/brandonhopkins/Projects/avalanchesales`

---

## Step 1 — Setup Agent

Spawn a **general-purpose** agent (`model="sonnet"`) with this exact prompt (substitute avalanchesales and today's date):

> Your only job is to set up the project folder.
>
> 1. Copy the entire directory `/Users/brandonhopkins/Projects/WEBSITETEMPLATE/` to `/Users/brandonhopkins/Projects/avalanchesales/`
> 2. Inside the new folder, rename `SITE_FACTS_TEMPLATE.md` to `SITE_FACTS.md`
> 3. In `CLAUDE.md`, replace `[Project Name]` with `avalanchesales` and `[YYYY-MM-DD]` with today's date
> 4. In `README.md`, replace every occurrence of `avalanchesales` with the actual client name
> 5. Verify the copy succeeded by listing all files in `/Users/brandonhopkins/Projects/avalanchesales/`
...
Spawn a **general-purpose** agent (`model="sonnet"`) with this exact prompt (substitute avalanchesales and DOMAIN):

> Your only job is to research avalanchesales and fully populate their SITE_FACTS.md.
>
...
>
> Download all logos, staff photos, gallery images, and program/product images to `/Users/brandonhopkins/Projects/avalanchesales/assets/` organized by subfolder: `logos/`, `team/`, `gallery/`, `products/`. For gallery pages or slideshows, enumerate every individual image URL and download each one. If an image cannot be downloaded, note its URL in SITE_FACTS.md instead.
>
> Fill out every section of `/Users/brandonhopkins/Projects/avalanchesales/SITE_FACTS.md`. No placeholder text or bracket fields should remain. If data is genuinely unavailable, write `Unknown` rather than leaving a blank. Do NOT fill in the Technology Stack section - leave all values as their template placeholders so the build agent can complete it.
>
...
Spawn a **general-purpose** agent (`model="sonnet"`) with this exact prompt (substitute avalanchesales):

> Your only job is to build and deploy the website.
>
> 1. Read `/Users/brandonhopkins/Projects/avalanchesales/SITE_FACTS.md` for all content and structure.
> 2. Scaffold a Next.js (App Router) + TypeScript + Tailwind CSS site in `/Users/brandonhopkins/Projects/avalanchesales/site/`
> 3. Build every page listed in the Site Structure section of SITE_FACTS.md.
> 4. Use all assets from `/Users/brandonhopkins/Projects/avalanchesales/assets/`
> 5. Run `npm install` then `npm run build`. Fix all errors before continuing.
> 6. Initialize a git repo at the project root `/Users/brandonhopkins/Projects/avalanchesales/` (not inside site/). Add a `.gitignore` that excludes `site/node_modules`. Commit all files.
> 7. Create a GitHub repo named `avalanchesales` under the `CalTech-Web` org and push: `gh repo create CalTech-Web/avalanchesales --public --source=. --remote=origin --push`
> 8. Deploy from the project root with the root directory set to `site`: `cd /Users/brandonhopkins/Projects/avalanchesales && npx vercel --yes --prod --name avalanchesales`
...
> 10. Trigger one more deploy to confirm the root directory setting works: `cd /Users/brandonhopkins/Projects/avalanchesales && npx vercel --yes --prod`
...
Run these four bash commands one at a time, in order. Wait for each to finish before running the next. Set a timeout for each task to 60 minutes. Substitute `avalanchesales` with the actual folder name.

```
WORK=/Users/brandonhopkins/Projects/avalanchesales/agents/design-agent MODEL="sonnet" MAX_LOOPS=5 bash ~/Projects/claude-lab/ENGINE/run.sh
```
```
WORK=/Users/brandonhopkins/Projects/avalanchesales/agents/cro-agent MODEL="sonnet" MAX_LOOPS=3 bash ~/Projects/claude-lab/ENGINE/run.sh
```
```
WORK=/Users/brandonhopkins/Projects/avalanchesales/agents/seo-agent MODEL="sonnet" MAX_LOOPS=5 bash ~/Projects/claude-lab/ENGINE/run.sh
```
```
WORK=/Users/brandonhopkins/Projects/avalanchesales/agents/pagespeed-agent MODEL="sonnet" MAX_LOOPS=2 bash ~/Projects/claude-lab/ENGINE/run.sh
```
...
Before spawning the agent: read the Site Structure section of `/Users/brandonhopkins/Projects/avalanchesales/SITE_FACTS.md` to get the actual page list and URL paths. Substitute avalanchesales with the real project name in the file path below.

Spawn a **visual-auditor** agent (`model="sonnet"`) with this prompt (substitute avalanchesales and VERCEL_URL, and paste the actual page URL list from SITE_FACTS.md):
...
>
> Save the complete findings to `/tmp/avalanchesales-audit.md`. Return when the file is written.

**Do not proceed to Step 6 until `/tmp/avalanchesales-audit.md` exists.**

---

## Step 6 — Fix Agent

Read `/tmp/avalanchesales-audit.md`. Spawn a **general-purpose** agent (`model="sonnet"`) with this exact prompt (substitute avalanchesales and paste the audit contents):

> Your only job is to fix every issue listed below. Make each fix in the codebase at `/Users/brandonhopkins/Projects/avalanchesales/site/`. After all fixes are made, commit, push, and deploy:
> `cd /Users/brandonhopkins/Projects/avalanchesales && npx vercel --yes --prod`
>
> [paste full contents of /tmp/avalanchesales-audit.md here]
>
...
Spawn a **general-purpose** agent (`model="sonnet"`) with this exact prompt (substitute avalanchesales and VERCEL_URL):
